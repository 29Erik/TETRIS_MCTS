...
namespace dr 
{
	namespace mcts 
	{
		...
		public:
			...
			Nodo* expandir()
			{
				...
				if (acciones.empty()) {
					...
					random_shuffle(acciones.begin(), acciones.end());
				}
		...
		private:
			...
			vector< Ptr > hijos;	// todos los hijos del nodo actual
			vector< Accion > acciones;			// posibles acciones para el estado actual
